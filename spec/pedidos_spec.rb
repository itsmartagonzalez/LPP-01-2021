RSpec.describe Cultivos do
    ...
    describe Pedidos do
        ...
        it "Se crea un objeto de tipo Pedido" do
            expect(@pedido).not_to eq(nil)
        end

        it "Escriba el método que permite representar el pedido de una cantidad determinada del producto hortaliza" do
            expect(@pedido.product(HortalizasEco.new("Acelga", 0.5, 30.0, 2.25, 2.00, 0, "Elevada"), :weight => "100")).to eq("\t[100 kilos] (Acelga - 2.25 - 2.0 - Elevada")
        end

        it "Escriba el método que permite representar la especificación de la permanencia de la solicitud de una hortaliza" do
            expect(@pedido.specification("Tiempo del contrato de la acelga", :time => "un año")).to eq("Tiempo del contrato de la acelga un año")
        end

        it "Se imprime bien el ticket del pedido" do
